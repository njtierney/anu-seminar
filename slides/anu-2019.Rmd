...
date: "<span style = 'font-size: 90%;'>ANU <br><br> Thursday 14th November, 2019 </span><br><br> `r icon::fa('link')` [bit.ly/njt-anu](https://bit.ly/njt-anu)<br><br>`r icon::fa('twitter')` [nj_tierney](https://twitter.com/nj_tierney)"
output:
...
Studied monocular occlusion and 3D perception of edges 
]
...
.large[
> If every psychologist in the world delivered gold standard smoking cessation therapy, and the rate of smoking would still increase. You need to change policy to make change. To make effective policy changes, you need to have good data, and do good statistics.
]
...
- Exploring the data?
- How???
- What does exploring the data even mean?
...
- Not much research on how we explore our methods and behaviour.
]
...
.pull-right.large[
  > Focus on building a bridge across a river. No focus on **how** we build the bridge, and the **tools** used.
]
...
Promoted by John Tukey, Frederick Mosteller, Bill Cleveland
]
...
.large[
- How many subsamples to look at? (2, 4, ... 16?)
- How many keys per subsample?
  - 153 keys into 16 subsamples = 9 each
...
How many keys per plot should I look at

--

How do I ensure that there are the same number of keys per plot
...
# Removing the distraction threshold means asking relevant questions
...
```{r show-facet-sample-print, ref.label = 'show-facet-sample', echo = FALSE, out.width = "100%"}
```
...
## In asking these questions we can solve something else interesting - can we re-order these facets in a meaningful way?
...
.large[
- `r icon::fa_industry()` using `sample_n_keys()` and `stratify_keys()`
]
...
```{r show-five-num-again, echo = TRUE}
heights %>%
  features(height_cm,
           feat_five_num)
```
...
# Can we look at some subsamples?
...
# Can we look at all subsamples arranged by residuals?
...
```{r small-sample-add-real-data, echo = TRUE, out.width = "50%"}
heights_sample +
  geom_point(aes(y = height_cm))  #<< add the original data
```
...
`r icon::fa('link')` [bit.ly/njt-anu](https://bit.ly/njt-anu)
